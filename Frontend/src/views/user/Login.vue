            ...
            <div v-if="isUserLogin" class="form-wrapper" key="user-form">
              <form @submit.prevent="loginUser">
                <div class="input-wrapper mb-3">
                  ...
                  <input 
                    id="username" 
                    v-model="userUsername.value.value" 
                    type="text" 
                    class="form-control" 
                    :class="{ 'is-invalid': userUsername.errorMessage.value }"
                    placeholder="Email/Số điện thoại/Tên đăng nhập"
                    @blur="userUsername.handleBlur"
                  />
                  <div v-if="userUsername.errorMessage.value" class="invalid-feedback d-block">
                    {{ userUsername.errorMessage.value }}
                  </div>
                </div>
                ...
                <div class="input-wrapper mb-4">
                  ...
                  <input 
                    ...
                    class="form-control"
                    :class="{ 'is-invalid': userPassword.errorMessage.value }"
                    placeholder="Mật khẩu"
                    @blur="userPassword.handleBlur"
                  />
                  ...
                  <div v-if="userPassword.errorMessage.value" class="invalid-feedback d-block">
                ...
                <BaseButton 
                  ...
                  class="mb-3"
                >
                  {{ isUserLoading ? 'Đang xử lý...' : 'Đăng Nhập' }}
                </BaseButton>
              ...
              <div class="social-buttons">
                 <BaseButton variant="outline-primary" class="btn-social facebook">
                  <i class="bi bi-facebook social-icon me-2"></i>
                  <span>Facebook</span>
                </BaseButton>
                <BaseButton variant="outline-secondary" class="btn-social google" @click="loginWithGoogle">
                  <i class="bi bi-google social-icon me-2"></i>
                  <span>Google</span>
                </BaseButton>
              </div>
            ...
            <div v-else class="form-wrapper" key="seller-form">
              <form @submit.prevent="loginSeller">
                <div class="input-wrapper mb-3">
                  ...
                  <input 
                    id="seller-username" 
                    v-model="sellerUsername.value.value" 
                    type="text" 
                    class="form-control"
                    :class="{ 'is-invalid': sellerUsername.errorMessage.value }"
                    placeholder="Email/Tên đăng nhập"
                    @blur="sellerUsername.handleBlur"
                  />
                  <div v-if="sellerUsername.errorMessage.value" class="invalid-feedback d-block">
                    {{ sellerUsername.errorMessage.value }}
                  </div>
                </div>
                ...
                <div class="input-wrapper mb-4">
                  ...
                  <input 
                    id="seller-password" 
                    v-model="sellerPassword.value.value" 
                    :type="sellerPasswordFieldType" 
                    class="form-control"
                    :class="{ 'is-invalid': sellerPassword.errorMessage.value }"
                    placeholder="Mật khẩu"
                    @blur="sellerPassword.handleBlur"
                  />
                  ...
                  <div v-if="sellerPassword.errorMessage.value" class="invalid-feedback d-block">
                    {{ sellerPassword.errorMessage.value }}
                  </div>
                </div>
                ...
                <BaseButton 
                  ...
                  class="mb-3"
                >
                  {{ isSellerLoading ? 'Đang xử lý...' : 'Đăng Nhập' }}
                </BaseButton>

                <BaseAlert 
                  v-model="showSellerError"
                  variant="danger" 
                  :message="sellerError"
                  dismissible
                  :auto-dismiss="5000"
...
import { BaseButton, BaseAlert, BaseInput } from '@/components/ui';
import { loginUserSchema, loginSellerSchema } from '@/validation/auth/loginValidate';

const router = useRouter();

// State
const loginMethod = ref('user');
const isUserPasswordVisible = ref(false);
const isSellerPasswordVisible = ref(false);
const userError = ref(null);
const showUserError = ref(false);
const isUserLoading = ref(false);
const sellerError = ref(null);
const showSellerError = ref(false);
const isSellerLoading = ref(false);

// Setup form validation cho User
...
const userUsername = useField('username', undefined, { form: userForm });
const userPassword = useField('passwords', undefined, { form: userForm });

// Setup form validation cho Seller
const sellerForm = useForm({
  validationSchema: loginSellerSchema,
  initialValues: {
    username: '',
    passwords: ''
  }
...
const sellerUsername = useField('username', undefined, { form: sellerForm });
const sellerPassword = useField('passwords', undefined, { form: sellerForm });

const isUserLogin = computed(() => loginMethod.value === 'user');
const userPasswordFieldType = computed(() => isUserPasswordVisible.value ? 'text' : 'password');
const sellerPasswordFieldType = computed(() => isSellerPasswordVisible.value ? 'text' : 'password');
...
const toggleUserPasswordVisibility = () => {
  isUserPasswordVisible.value = !isUserPasswordVisible.value;
};

const toggleSellerPasswordVisibility = () => {
  isSellerPasswordVisible.value = !isSellerPasswordVisible.value;
};

const loginUser = userForm.handleSubmit(async (values) => {
  ...
  try {
    ...
    if (res.data.role !== 'ROLE_USER') {
      userError.value = "Tài khoản của bạn không phải tài khoản người dùng.";
    ...
    userError.value = "Sai tên đăng nhập hoặc mật khẩu!";
    showUserError.value = true;
  } finally {
...
const loginSeller = sellerForm.handleSubmit(async (values) => {
  isSellerLoading.value = true;
  sellerError.value = null;
  showSellerError.value = false;
  ...
  try {
    ...
    if (role === 'ROLE_ADMIN' || role === 'ROLE_STAFF') {
      ...
      sellerError.value = "Bạn không có quyền truy cập Kênh Người Bán.";
      showSellerError.value = true;
    }
  } catch (err) {
    sellerError.value = "Sai tên đăng nhập hoặc mật khẩu!";
    showSellerError.value = true;
  } finally {
...
if (errorParam) {
  userError.value = decodeURIComponent(errorParam);
  showUserError.value = true;
  ...
  if (token) {
    ...
    params.forEach((value, key) => {
      googleUserData[key] = decodeURIComponent(value || "");
    });
    saveAuthData(googleUserData);
    router.push("/");
  }
}
