  ...
  },
  {
    ...
    beforeEnter: (to, from, next) => {
      ...
      if (!token) {
        if (
          confirm(
            "Bạn cần đăng nhập để xem thông tin tài khoản. Bạn có muốn đăng nhập ngay không?"
          )
        ) {
...
router.beforeEach(async (to, from, next) => {
  ...
  const isSellerOnUserPage =
    (userRole === "ROLE_ADMIN" || userRole === "ROLE_STAFF") &&
    !to.path.startsWith("/admin");
  const isUserOnAdminPage =
    userRole === "ROLE_USER" && to.path.startsWith("/admin");
  ...
  if (isSellerOnUserPage || isUserOnAdminPage) {
    ...
    try {
      ...
      console.error(
        "[AUTH] Server logout failed, but proceeding with client-side logout.",
        error
      );
    }
